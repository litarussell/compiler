...
int main() {
	...
	// fin = fopen(fname, "r");
	fin = fopen("/Users/lita/_work/project/compiler/pl0/test.pl0", "r");
...
void init() {
	...
	/* 设置语句开始符号集 */
	statbegsys[beginsym] = true;	// 赋值
	statbegsys[callsym] = true;		// 调用
...
int block(int lev, int tx, bool* fsys) {
	...
	int tx0;                /* 保留初始tx */
	int cx0;                /* 保留初始cx */
	/* 在下级函数的参数中，符号集合均为值参，但由于使用数组实现，传递进来的是指针，
	...
	do {
		...
		if (sym == varsym) { /* 收到变量声明符号，开始处理变量声明 */
			...
			/* the original do...while(sym == ident) is problematic, thanks to calculous */
			/* do {  */
			vardeclarationdo(&tx, lev, &dx); // 变量定义处理
			while (sym == comma) {
				getsymdo;
